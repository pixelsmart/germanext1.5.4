		...
		if (Validate::isLoadedObject($this->context->cart) && $this->context->cart->OrderExists() == false)
		{
			...
			foreach ($package_list as $id_address => $packageByAddress)
				foreach ($packageByAddress as $id_package => $package)
				{
					...
					$payment_fees = $order->getOrderPaymentFeeDetails(
						(
							$order->total_products_wt + $order->total_shipping_tax_incl + $order->total_wrapping_tax_incl - abs($order->total_discounts_tax_incl)
						),
						isset($this->name) ? $this->name : false,
						$package['product_list']
					);
					
					if ($payment_fees)
					{
						foreach ($payment_fees as $property => $value)
						{
					...
					Order::addCompoundTaxesToTaxArray($taxDetails, array($order->total_shipping_tax_incl, $order->total_payment_tax_incl, $order->total_wrapping_tax_incl, -abs($order->total_discounts_tax_incl)));

					$order->total_paid_tax_excl = 0;

					foreach ($taxDetails as $taxDetail)
					{
						$order->total_paid_tax_excl+= (float)$taxDetail['total_net'];
					}
					
					$order->total_paid_tax_excl = Tools::ps_round($order->total_paid_tax_excl, 2);
			...
			if ($order_status->logable)
			{
				...
				if (!$order->addOrderPayment($amount_paid, (int)PaymentCost::getModuleIdByPaymentId((int)$this->context->cart->id_payment), $transaction_id))
				{
				...
				if (!$order_creation_failed && isset($order->id))
				{
					...
					foreach ($products as $key => $product)
					{
						...
						if (isset($customized_datas[$product['id_product']][$product['id_product_attribute']]))
						{
							...
							'<tr style="background-color: '.($key % 2 ? '#DDE2E6' : '#EBECEE').';">
								...
								<td style="padding: 0.6em 0.4em;width: 15%;">' . $availability . '</td>
								<td style="padding: 0.6em 0.4em;width: 15%;">'.Tools::displayPrice(Product::getTaxCalculationMethod() == PS_TAX_EXC ?  Tools::ps_round($price, 2) : $price_wt, $this->context->currency, false).'</td>
								<td style="padding: 0.6em 0.4em;width: 10%;">'.$customization_quantity.'</td>
								<td style="padding: 0.6em 0.4em;width: 15%;">'.Tools::displayPrice($customization_quantity * (Product::getTaxCalculationMethod() == PS_TAX_EXC ? Tools::ps_round($price, 2) : $price_wt), $this->context->currency, false).'</td>
							</tr>';
						}
						...
						if (!$customization_quantity || (int)$product['cart_quantity'] > $customization_quantity)
							...
							'<tr style="background-color: '.($key % 2 ? '#DDE2E6' : '#EBECEE').';">
								...
								<td style="padding: 0.6em 0.4em;width: 15%;">'.Tools::displayPrice(((int)$product['cart_quantity'] - $customization_quantity) * (Product::getTaxCalculationMethod() == PS_TAX_EXC ? Tools::ps_round($price, 2) : $price_wt), $this->context->currency, false).'</td>
							</tr>';
					...
					foreach ($cart_rules as $cart_rule)
					{
						...
						*/
						...
						$cart_rules_list .= '
						<tr>
							<td colspan="5" style="padding:0.6em 0.4em;text-align:right">'.Tools::displayError('Voucher name:').' '.$cart_rule['obj']->name.'</td>
					...
					if (sizeof($taxDetails))
					{
						$i = 0;
						foreach ($taxDetails as $taxName => $taxData)
						{
							$taxDetailsStr.= '
								<tr style="background-color: '.($i % 2 ? '#DDE2E6' : '#EBECEE').';">
									<td style="padding: 0.6em 0.4em; text-align: right;" colspan="3">' . $taxName . ' (' . number_format($taxData['rate'],1,',','').'%)</td>
									<td style="padding: 0.6em 0.4em; text-align: right;">' . Tools::displayPrice($taxData['total_net'], $currency, false) . '</td>
									<td style="padding: 0.6em 0.4em; text-align: right;">' . Tools::displayPrice($taxData['total'], $currency, false) . '</td>
									<td style="padding: 0.6em 0.4em; text-align: right;">' . Tools::displayPrice($taxData['total_vat'], $currency, false) . '</td>
								</tr>';
							$i++;
						}
					}
					...
					if ($id_order_state != Configuration::get('PS_OS_ERROR') && $id_order_state != Configuration::get('PS_OS_CANCELED') && $this->context->customer->id)
					{
						...
						'{total_payment}' => Tools::displayPrice($order->total_payment_tax_incl, $this->context->currency, false),
						'{total_paid}' => Tools::displayPrice($order->total_paid, $this->context->currency, false),
						'{total_products}' => Tools::displayPrice($order->total_products_wt, $this->context->currency, false),
						'{total_discounts}' => Tools::displayPrice(-abs($order->total_discounts), $this->context->currency, false),
