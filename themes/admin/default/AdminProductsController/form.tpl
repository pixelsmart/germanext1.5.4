				...
				setup : function(ed) {
					...
					ed.onKeyUp.add(function(ed, e) {
						...
						if (max != 'none')
						{
							...
							rest = max - count;
							if (rest < 0)
								textarea.parent('div').find('span.counter').html('<span style="color:red;">{l s='Maximum' mod='germanext'} '+max+' {l s='characters' mod='germanext'} : '+rest+'</span>');
							else
...
{block name="defaultForm"}
	...
	<script type="text/javascript">
		// <![CDATA[
			ThickboxI18nImage = "{l s='Image' mod='germanext'}";
			ThickboxI18nOf = "{l s='of' mod='germanext'}";
			ThickboxI18nClose = "{l s='Close' mod='germanext'}";
			ThickboxI18nOrEscKey = "{l s='(or "Esc")' mod='germanext'}";
			ThickboxI18nNext = "{l s='Next >' mod='germanext'}";
			ThickboxI18nPrev = "{l s='< Previous' mod='germanext'}";
			tb_pathToImage = "../img/loadingAnimation.gif";
		//]]>
	</script>

	<div id="product-tab-content-wait" style="display:none"><div id="loading">{l s='Loading...' mod='germanext'}</div></div>
	...
	<form id="product_form" action="{$form_action}" method="post" enctype="multipart/form-data" name="product" style="display:none;">
		...
		{if !$product->active}
			<div class="warn draft" >
				<p>
					...
					{l s='Your product will be saved as a draft' mod='germanext'}</span>
					<span style="float:right"><a href="#" class="button" style="display: block" onclick="submitAddProductAndPreview()" >{l s='Save and preview' mod='germanext'}</a></span>
		...
		<input id="product_form_submit_btn"  type="submit" value="{l s='Save' mod='germanext'}" name="submitAddproduct" class="button" />
	</form>
