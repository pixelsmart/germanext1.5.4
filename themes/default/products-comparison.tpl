...
{capture name=path}{l s='Product Comparison' mod='germanext'}{/capture}

{include file="$tpl_dir./breadcrumb.tpl"}
<h1>{l s='Product Comparison' mod='germanext'}</h1>
...
<div class="products_block">
	<table id="product_comparison">
		...
		{foreach from=$products item=product name=for_products}
			...
			<td width="{$width}%" class="ajax_block_product comparison_infos">
				...
				<p class="s_title_block">
					<a href="{$product->getLink()}" title="{$product->name|truncate:32:'...'|escape:'htmlall':'UTF-8'}">{$product->name|truncate:27:'...'|escape:'htmlall':'UTF-8'}</a>
				</p>
				<div class="product_desc"><a href="{$product->getLink()}">{$product->description_short|strip_tags|truncate:60:'...'}</a></div>
				<a class="lnk_more" href="{$product->getLink()}" title="{l s='View' mod='germanext'}">{l s='View' mod='germanext'}</a>
				...
				<div class="comparison_product_infos">
					...
					{if isset($product->show_price) && $product->show_price && !isset($restricted_country_mode) && !$PS_CATALOG_MODE}
						...
						{if !empty($product->unity) && $product->unit_price_ratio > 0.000000}
							...
							<p class="comparison_unit_price">{convertPrice price=$unit_price} {l s='per %s' mod='germanext' sprintf=$product->unity|escape:'htmlall':'UTF-8'}</p>
						{else}
						&nbsp;
						{/if}
					{/if}
					...
					{if !(($product->quantity <= 0 && !$product->available_later) OR ($product->quantity != 0 && !$product->available_now) OR !$product->available_for_order OR $PS_CATALOG_MODE)}
						...
						<span id="availability_value"{if $product->quantity <= 0} class="warning-inline"{/if}>
							{if $product->quantity <= 0}
								...
								{else}
									{l s='This product is no longer in stock' mod='germanext'}
								{/if}
							{else}
				...
				<a class="cmp_remove" href="{$link->getPageLink('products-comparison', true)}" rel="ajax_id_product_{$product->id}">{l s='Remove' mod='germanext'}</a>
				</div>
			</td>
		{/foreach}
		</tr>
		...
		{else}
			<tr>
				<td></td>
				<td colspan="{$products|@count + 1}">{l s='No features to compare' mod='germanext'}</td>
			</tr>
		{/if}
...
{else}
	<p class="warning">{l s='There are no products selected for comparison' mod='germanext'}</p>
{/if}
