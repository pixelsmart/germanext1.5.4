...
<p>{l s='Here are the orders you\'ve placed since your account was created.' mod='germanext'}.</p>
...
<div class="block-center" id="block-history">
	...
	<table id="order-list" class="std">
		<thead>
			<tr>
				<th class="first_item">{l s='Order reference' mod='germanext'}</th>
		...
		{foreach from=$orders item=order name=myLoop}
			<tr class="{if $smarty.foreach.myLoop.first}first_item{elseif $smarty.foreach.myLoop.last}last_item{else}item{/if} {if $smarty.foreach.myLoop.index % 2}alternate_item{/if}">
				...
				{if (isset($order.invoice) && $order.invoice && isset($order.invoice_number) && $order.invoice_number) && isset($invoiceAllowed) && $invoiceAllowed == true}
					<a href="{$link->getPageLink('pdf-invoice', true, NULL, "id_order={$order.id_order}")}" title="{l s='Invoice' mod='germanext'}" class="_blank"><img src="{$img_dir}icon/pdf.gif" alt="{l s='Invoice' mod='germanext'}" class="icon" /></a>
					<a href="{$link->getPageLink('pdf-invoice', true, NULL, "id_order={$order.id_order}")}" title="{l s='Invoice' mod='germanext'}" class="_blank">{l s='PDF' mod='germanext'}</a>
				{else}-{/if}
				</td>
