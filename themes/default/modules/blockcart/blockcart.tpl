...
var customizationIdMessage = '{l s='Customization #' mod='germanext' js=1}';
var removingLinkText = '{l s='remove this product from my cart' mod='germanext' js=1}';
var freeShippingTranslation = '{l s='Free shipping!' mod='germanext' js=1}';
var freeProductTranslation = '{l s='Free!' mod='germanext' js=1}';
var delete_txt = '{l s='Delete' mod='germanext'}';
var img_dir = '{$img_dir}';
</script>
{/if}
...
<div id="cart_block" class="block exclusive">
	<h4>
		<a href="
			{if $order_process == 'order-opc'}
                {$link->getPageLink("$order_process.php", true, null, 'shipping_cart=1')}
            {elseif $order_process == 'order'}
                {$link->getPageLink("$order_process.php", true, null, 'step=1')}
            {else}
                {$link->getPageLink("$order_process.php", true)}
            {/if}">{l s='Cart' mod='germanext'}</a>
		{if $ajax_allowed}
	...
	<div id="cart_block_summary" class="{if isset($colapseExpandStatus) && $colapseExpandStatus eq 'expanded' || !$ajax_allowed || !isset($colapseExpandStatus)}collapsed{else}expanded{/if}">
		...
		<span class="ajax_cart_product_txt_s" {if $cart_qties <= 1}style="display:none"{/if}>{l s='products' mod='germanext'}</span>
		<span class="ajax_cart_product_txt" {if $cart_qties > 1}style="display:none"{/if}>{l s='product' mod='germanext'}</span>
		...
		<span class="ajax_cart_no_product" {if $cart_qties != 0}style="display:none"{/if}>{l s='(empty)' mod='germanext'}</span>
	</div>
	...
	{if $products}
		...
		{foreach from=$products item='product' name='myLoop'}
			...
			<dd id="cart_block_combination_of_{$product.id_product}{if $product.id_product_attribute}_{$product.id_product_attribute}{/if}_{$product.id_address_delivery|intval}" class="{if $smarty.foreach.myLoop.first}first_item{elseif $smarty.foreach.myLoop.last}last_item{else}item{/if}">
				<a href="{$link->getProductLink($product, $product.link_rewrite, $product.category, null, null, $product.id_shop, $product.id_product_attribute)}" title="{l s='Product detail' mod='germanext'}">{$product.attributes_small}</a>
			{/if}
				...
				<ul class="cart_block_customizations" id="customization_{$productId}_{$productAttributeId}">
					{foreach from=$customizedDatas.$productId.$productAttributeId[$product.id_address_delivery] key='id_customization' item='customization' name='customizations'}
						<li name="customization">
							...
							{else}
							{l s='Customization #%d:' sprintf=$id_customization|intval mod='germanext'}
							{/if}
						</li>
					{/foreach}
				</ul>
	...
	{/if}
		<p {if $products}class="hidden"{/if} id="cart_block_no_products">{l s='No products' mod='germanext'}</p>
	{if $discounts|@count > 0}
			...
			<tbody>
					{foreach from=$discounts item=discount}
						...
						<tr class="bloc_cart_voucher" id="bloc_cart_voucher_{$discount.id_discount}">
							...
							<td class="delete"><a class="delete_voucher" href="{$link->getPageLink('$order_process.php', true)}?deleteDiscount={$discount.id_discount}" title="{l s='Delete' mod='germanext'}"><img src="{$img_dir}icon/delete.gif" alt="{l s='Delete' mod='germanext'}" class="icon" /></a></td>
						</tr>
						{/if}
					{/foreach}
			</tbody>
		...
		<p id="cart-prices">
			{if $show_wrapping}
				...
				<span>{l s='Wrapping' mod='germanext'}</span>
				<br/>
			{/if}
			{if $show_tax && isset($tax_cost)}
				<span id="cart_block_tax_cost" class="price ajax_cart_tax_cost">{$tax_cost}</span>
				<span>{l s='Tax' mod='germanext'}</span>
				<br/>
			{/if}
			<span id="cart_block_total" class="price ajax_block_cart_total">{$total}</span>
			<span>{l s='Total' mod='germanext'}</span>
			<br />
			{if $show_shipping}
			<span id="cart_block_shipping_cost" class="price ajax_cart_shipping_cost">{$shipping_cost}</span>
			<span>{l s='Shipping' mod='germanext'}</span>
			<br />
			{else}
				<span class="gn_shipping gn_block"> {l s='plus' mod='germanext'}
				<span>{if isset($CMS_SHIPPING_LINK) && $CMS_SHIPPING_LINK}<a href="{$CMS_SHIPPING_LINK}" class="iframe fancybox">{/if}{l s='shipping costs' mod='germanext'}{if isset($CMS_SHIPPING_LINK) && $CMS_SHIPPING_LINK}</a>{/if}</span>
				<br />
			{/if}
			{if isset($ustgdisp) && $ustgdisp.incart}
				<span class="ustg">{l s='tax not set in the invoice' mod='germanext'}</span>
			{/if}
		</p>
		{if $use_taxes && $display_tax_label == 1 && $show_tax}
			{if $priceDisplay == 0}
				<p id="cart-price-precisions">
					{l s='Prices are tax included' mod='germanext'}
				</p>
			{/if}
			{if $priceDisplay == 1}
				<p id="cart-price-precisions">
					{l s='Prices are tax excluded' mod='germanext'}
				</p>
			{/if}
		{/if}
		<p id="cart-buttons">
			{if $order_process == 'order'}<a href="{$link->getPageLink("$order_process.php?shipping_cart=1", true)}" class="button_small" title="{l s='Cart' mod='germanext'}">{l s='Cart' mod='germanext'}</a>{/if}
			<a href="
			{if $order_process == 'order-opc'}
                {$link->getPageLink("$order_process.php", true, null, 'shipping_cart=1')}
            {elseif $order_process == 'order'}
            ...
            {else}
                {$link->getPageLink("$order_process.php", true)}
			{/if}" id="button_order_cart" class="exclusive{if $order_process == 'order-opc'}_large{/if}" title="{l s='Check out' mod='germanext'}">{l s='Check out' mod='germanext'}</a>
		</p>
	</div>
	</div>
</div>
