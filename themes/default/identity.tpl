...
{capture name=path}<a href="{$link->getPageLink('my-account', true)}">{l s='My account' mod='germanext'}</a><span class="navigation-pipe">{$navigationPipe}</span>{l s='Your personal information' mod='germanext'}{/capture}
{include file="$tpl_dir./breadcrumb.tpl"}

<h1>{l s='Your personal information' mod='germanext'}</h1>

{include file="$tpl_dir./errors.tpl"}

{if isset($confirmation) && $confirmation}
	<p class="success">
		{l s='Your personal information has been successfully updated.' mod='germanext'}
		{if isset($pwd_changed)}<br />{l s='Your password has been sent to your email:' mod='germanext'} {$email|escape:'htmlall':'UTF-8'}{/if}
	</p>
{else}
	<h3>{l s='Please be sure to update your personal information if it has changed.' mod='germanext'}</h3>
	<p class="required"><sup>*</sup>{l s='Required field' mod='germanext'}</p>
	<form action="{$link->getPageLink('identity', true)}" method="post" class="std">
		<fieldset>
			...
			<p class="required text">
				<label for="old_passwd">{l s='Current Password' mod='germanext'} <sup>*</sup></label>
				<input type="password" name="old_passwd" id="old_passwd" />
			</p>
			<p class="password">
				<label for="passwd">{l s='New Password' mod='germanext'}</label>
				<input type="password" name="passwd" id="passwd" />
			</p>
			<p class="password">
				<label for="confirmation">{l s='Confirmation' mod='germanext'}</label>
				<input type="password" name="confirmation" id="confirmation" />
			</p>
			<p class="select">
				<label>{l s='Date of Birth' mod='germanext'}</label>
			...
			{/if}
			<p class="submit">
				<input type="submit" class="button" name="submitIdentity" value="{l s='Save' mod='germanext'}" />
			</p>
			<p id="security_informations">
				{l s='[Insert customer data privacy clause here, if applicable]' mod='germanext'}
			</p>
		</fieldset>
	</form>
{/if}
...
<ul class="footer_links">
	<li><a href="{$link->getPageLink('my-account', true)}"><img src="{$img_dir}icon/my-account.gif" alt="" class="icon" /></a><a href="{$link->getPageLink('my-account', true)}">{l s='Back to your account' mod='germanext'}</a></li>
