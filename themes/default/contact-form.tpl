...
{capture name=path}{l s='Contact' mod='germanext'}{/capture}
{include file="$tpl_dir./breadcrumb.tpl"}

<h1>{l s='Customer service' mod='germanext'} - {if isset($customerThread) && $customerThread}{l s='Your reply' mod='germanext'}{else}{l s='Contact us' mod='germanext'}{/if}</h1>

{if isset($confirmation)}
	<p>{l s='Your message has been successfully sent to our team.' mod='germanext'}</p>
...
{else}
	<p class="bold">{l s='For questions about an order or for more information about our products' mod='germanext'}.</p>
	...
	<form action="{$request_uri|escape:'htmlall':'UTF-8'}" method="post" class="std" enctype="multipart/form-data">
		...
		{if !$PS_CATALOG_MODE}
			...
			<p class="text select">
				<label for="id_order">{l s='Order reference' mod='germanext'}</label>
		...
		{if $fileupload == 1}
			<p class="text">
			<label for="fileUpload">{l s='Attach File' mod='germanext'}</label>
