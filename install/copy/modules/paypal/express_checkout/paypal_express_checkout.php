...
class PaypalExpressCheckout extends Paypal
{
	...
	public function setExpressCheckout()
	{
		...
		$pf_rate = ((float)($this->context->cart->getOrderTotal(true, Cart::BOTH_WITHOUT_SHIPPING)) 
					+ $this->context->cart->getOrderTotal(true, Cart::ONLY_PAYMENT)) / $this->context->cart->getOrderTotal(true, Cart::BOTH_WITHOUT_SHIPPING);

		foreach ($this->product_list as $k => $v) {
			$this->product_list[$k]['price_wt'] = Tools::ps_round($this->product_list[$k]['price_wt'] * $pf_rate, 2);
        }
