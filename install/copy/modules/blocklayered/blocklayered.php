	...
	public function getProductByFilters($selected_filters = array())
	{
		...
		else
		{
			...
			LEFT JOIN `'._DB_PREFIX_.'tax_lang` tl
				...
				AND tl.`id_lang` = '.(int)($cookie->id_lang).')
	...
	public function ajaxCall()
	{
		...
		else
		{
			if (Module::isInstalled('germanext'))
			{
				...
				$smarty->assign('CMS_SHIPPING_LINK', Germanext::getCmsLink((int)Configuration::get('PS_CMS_ID_DELIVERY'), (int)$cookie->id_lang));
				$smarty->assign('germanext_tpl', GN_THEME_PATH);
				$product_list = $smarty->fetch(GN_THEME_PATH . 'product-list.tpl');
			}
			else
			{
