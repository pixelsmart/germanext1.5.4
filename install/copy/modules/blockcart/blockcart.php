...
class BlockCart extends Module
{
	...
	public function assignContentVars(&$params)
	{
		...
		$totalToPay = $params['cart']->getOrderTotal($useTax, isset($customer) ? Cart::BOTH : Cart::BOTH_WITHOUT_SHIPPING);

		if ($useTax && Configuration::get('PS_TAX_DISPLAY') == 1)
		{
			$totalToPayWithoutTaxes = $params['cart']->getOrderTotal(false, isset($customer) ? Cart::BOTH : Cart::BOTH_WITHOUT_SHIPPING);
