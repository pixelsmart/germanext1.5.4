...
class gn_masterpayment extends GN_PaymentManager
{
	...
	public function callPayment($params)
	{
		...
		$controller->addCSS(__PS_BASE_URI__.'modules/masterpayment/views/css/gateway.css');

		$context->smarty->assign(array(
			'valid_currency' => in_array($currency->iso_code, $valid_currencies),
			'API_PARAMS'     => $api->getParams(),
			'MP_GATEWAY_URL' => $cfg['MP_GATEWAY_URL'],
			'MP_MODE'        => $cfg['MP_MODE']
		));
	
		$controller->setTemplate(dirname(__FILE__) . '/form.tpl');
	...
	public function collectPaymentData() {
		...
		if ( ! Configuration::get('MB_PARAMETERS') || ! Configuration::get('MB_PARAMETERS_2') || (Configuration::get('MB_LOCAL_METHODS') == '' && Configuration::get('MB_INTER_METHODS') == '')) {
			return;
		}
	...
	public function presentPaymentOptions($paymentData) {
		...
		foreach ($payment_methods as $code => $method) {
			$context->smarty->assign('embedScript', $i == 0);
			...
			$result[] = $context->smarty->fetch(dirname(__FILE__) . '/masterpayment.tpl');
